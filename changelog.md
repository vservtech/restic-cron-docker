...
- **Feature: Multiple pg_restore versions**
  - Added `pg_restore16` and `pg_restore17` to `PATH`
  - Enables restoring backups to specific PostgreSQL versions
...
- **Feature: Multiple pg_dump versions**
  - Added PostgreSQL 16 and 17 client utilities
  - Added `pg_dump16` and `pg_dump17` to `PATH`
  - Added `pg_dump16`/`pg_dump17` usage note to README
...
**Included tools:**
```
restic version: restic 0.18.0 compiled with go1.24.8 on linux/arm64
